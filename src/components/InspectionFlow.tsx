...
function groupBySection(items: ChecklistItemDef[]): [ChecklistSection, ChecklistItemDef[]][] {
  const order: ChecklistSection[] = ['exterior', 'interior', 'posttrip'];
  return order
...
export function InspectionFlow({ vehicle, type, onDone }: InspectionFlowProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      const { inspection } = await submitInspection(vehicle.id, type, fullAnswers, notes.trim() || undefined);
      ...
      <div className="screen">
        <header className="screen__header">
          ...
          <p className="screen__subtitle vehicle-meta">
            <span className="vehicle-meta__plate">{vehicle.plate}</span> · {CURRENT_DRIVER.name} · {dateLabel}{' '}
            {timeLabel}
          </p>
        </header>
